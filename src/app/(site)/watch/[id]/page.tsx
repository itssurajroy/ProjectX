...
const extractHianimeEpId = (id: string | null): string | null => {
  if (!id) return null;
...
function WatchPageComponent() {
  ...
  const hianimeEpId = useMemo(() => {
      if (!currentEpisode) return null;
      return extractHianimeEpId(currentEpisode.episodeId);
  }, [currentEpisode]);
  ...
  const handleCreateW2G = async () => {
    toast.error('Watch Together is temporarily disabled.');
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mt-6">
        ...
        <div className={cn("lg:col-span-6 space-y-4", isFocusMode && "relative z-40")}>
            <div className="aspect-video w-full overflow-hidden rounded-lg bg-black">
                {currentEpisode && hianimeEpId ? (
                    <AnimePlayer 
                        hianimeEpisodeId={hianimeEpId}
          ...
          {about.info.id && <CommentsContainer animeId={about.info.id} animeName={about.info.name} episodeId={currentEpisode?.episodeId} episodeNumber={currentEpisode?.number} />}
        </div>
