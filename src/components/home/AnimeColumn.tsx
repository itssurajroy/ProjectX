...
export default function AnimeColumn({ title, animes, category }: { title: string, animes: AnimeBase[] | undefined, category: string }) {
    ...
    return (
        <div className="space-y-4">
            <h3 className="text-title font-bold border-l-4 border-primary pl-3">{title}</h3>
            <div className="space-y-3">
                {animes.slice(0, 7).map((anime, index) => {
                    const animeTitle = language === 'romaji' && anime.jname ? anime.jname : anime.name;
                    return (
                        <Link key={`${anime.id}-${index}`} href={`/anime/${anime.id}`} className="flex items-center gap-3 group p-1 rounded-md hover:bg-muted/50 transition-colors">
